 ...
 * @param {Array} [options.tableFields=['level', 'msg', 'meta', 'app']] - array of the table fields
...
class Postgres extends Transport {
  constructor(options = {}) {
    ...
    // JEFF - added to accomodate app source of log
   ...
   * @param {string} app App source of log
  ...
  log(args, callback) {
    ...
    const { level, message, meta, app} = args;
    if (!callback) {
      callback = () => {};
    }
    ...
    if (sqlStatement) {
      sqlStatement = {
        text: sqlStatement,
        values: ['now()', level, message, meta, app]
      };
    }
    ...
    return pool.connect().then(client => client
      .query(sqlStatement || table.insert(table.timestamp.value('NOW()'), table.level.value(level), table.message.value(message), table.meta.value(meta)).toQuery(), table.app.value(app))
      .then(() => {
        client.release();
